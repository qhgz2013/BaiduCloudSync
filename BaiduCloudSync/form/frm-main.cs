...
namespace BaiduCloudSync
{
    public partial class frmMain : Form
    {
        ...
        private FileListCacher _file_list;
        ...
        private void __update_treeview_data(string path = null)
        {
            ...
            {
                cur_path += paths[i] + "/";
                var files = _file_list.GetFileList(cur_path).ToList();
        ...
        private void __update_listview_data(string path = null, SortBase sort_type = SortBase.FileName, bool asc = true)
        {
            ...
            try
            {
                file_list = _file_list.GetFileList(path);
            }
            catch (Exception)
            {
                Invoke(new NoArgSTA(delegate
            ...
            _asyncCall(delegate
            {
                lock (__thd_lck)
                {
                    ...
                    if (node.Nodes.ContainsKey("<temp_node>"))
                    {
                        ...
                        var new_files = _file_list.GetFileList(__get_dir_path_from_node(node));
        ...
        {
            var data = _file_list.GetData(path);
            if (data.IsDir)
            {
                var files = _file_list.GetFileList(path);
                foreach (var item in files)
                {
            ...
            {
                _pcsAPI.ConvertToSymbolLink(data.Path, data.FS_ID);
            }
        }
        //UI: 创建文件链接
        ...
        private void delete_path(IEnumerable<string> paths)
        {
            ...
            var parent_path = FileListCacher.GetParentDir(path);
            _file_list.RemoveFileCache(parent_path);
        }
        //UI: 删除文件
        ...
        private void treeView_DirList_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            var node = treeView_DirList.HitTest(e.Location);
            if (node.Node != null)
            {
                var path = __get_dir_path_from_node(node.Node);
                __update_listview_data(path, __sortType, __asc);
            }
        }
        //UI: 属性
        private void 属性ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (selected_item == null || selected_item.Count == 0)
            {
                ...
                {
                    //root dir
                    property_form = new frmProperty(_file_list.GetFileList("/"), _file_list);
                }
                else
                {
                    //normal dir
                    property_form = new frmProperty(new ObjectMetadata[] { _file_list.GetData(__current_listview_path) }, _file_list);
                }
            }
            else
            {
                ...
                {
                    if (item.ImageKey == ":/dir/")
                        ls.Add(_file_list.GetData(__current_listview_path + item.Text + "/"));
                    else
                        ls.Add(_file_list.GetData(__current_listview_path + item.Text));

                }
                property_form = new frmProperty(ls, _file_list);
            }
        ...
        private void recreate_symbollink(string path)
        {
            ...
            {
                _pcsAPI.ConvertFromSymbolLink(data.Path, data.FS_ID);
            }
        }
        //UI: 生成文件
        ...
        private void _download_files(string path, string save_path)
        {
            ...
            var file_data = _file_list.GetFileList(path);
        ...
        private void 下载ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            foreach (ListViewItem item in selected_item)
            {
                data_list.Add(_file_list.GetData(node_path + item.Text + (item.ImageKey == ":/dir/" ? "/" : "")));
            }
                ...
                _asyncCall(delegate
                {
                    lock (__thd_lck)
                    {
                        ...
                        Invoke(new NoArgSTA(delegate
                        {
                            ...
                            if (node != null) treeView_DirList.SelectedNode = node;
                        }));
                    }
                }, "获取文件信息...");
        }
        ...
        private void _on_upload_finished(object sender, UploadResultEventArgs e)
        {
            _asyncCall(delegate
            {
                var parent = FileListCacher.GetParentDir(e.RemotePath);
                _file_list.RemoveFileCache(parent);
                刷新EToolStripMenuItem_Click(sender, e);
            }, "更新文件信息...");
        }
            ...
            _asyncCall(delegate
            {
                lock (__thd_lck)
                {
                    foreach (var local_path in local_paths)
                    {
                        ...
                        var new_class = new Uploader(_pcsAPI, remote_path, local_path, o);
        ...
        private void upload_files(string local_path, string remote_path)
        {
            ...
            foreach (var item in dir_info.GetFiles())
            {
                ...
                if (data.FindIndex(x => { return x.Path == remote_path + item.Name; }) != -1)
                {
                    ...
                    Invoke(new NoArgSTA(delegate
                    {
                        var result = MessageBox.Show(this, "文件已存在，是否覆盖（否则保存为新的文件）:\r\n" + remote_path + "/" + item.Name, "文件已存在", MessageBoxButtons.YesNoCancel);
                        if (result == DialogResult.Cancel) cancelled = true;
                        else if (result == DialogResult.Yes) o = ondup.overwrite;
                        else if (result == DialogResult.No) o = ondup.newcopy;
                    }));
                ...
                var new_class = new Uploader(_pcsAPI, remote_path + item.Name, item.FullName, o);
                new_class.TaskFinished += _on_upload_finished;
                uploadTransferList1.AddTask(new_class);
            }
            foreach (var item in dir_info.GetDirectories())
            {
                upload_files(local_path + item.Name + "/", remote_path + item.Name + "/");
            }
        }
        //UI: 上传文件夹
        ...
        private void 新建文件夹WToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            _asyncCall(delegate
            {
                lock (__thd_lck)
                {
                    ...
                    var data = _pcsAPI.CreateDirectory(path);
        ...
        private void 重命名MToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            _asyncCall(delegate
            {
                lock (__thd_lck)
                {
                    ...
                    var data = _pcsAPI.Rename(path, frm.FileName);
        ...
        private void 粘贴VToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            _asyncCall(delegate
            {
                ...
                {
                    if (_is_copy)
                        _pcsAPI.CopyPath(src_list, dest_list);
                    else
                        _pcsAPI.MovePath(src_list, dest_list);
                    foreach (var item in src_parent_list) { _file_list.RemoveFileCache(item); }
        ...
        {
            List<ObjectMetadata> delete_list;
            List<TrackedData> upload_list;
            if (UploadFileDir.ShowDialog() != DialogResult.OK) return;
            ...
            _asyncCall(delegate
            {
                _pcsAPI.GetSyncUpData(local_path, remote_path, _local_file_list, _file_list, out delete_list, out upload_list, true);
                //delete path
                ...
                _pcsAPI.DeletePath(delete_path);
                ...
                foreach (var item in upload_list)
                {
                    if (item.IsDir)
                    {
                        ...
                        _pcsAPI.CreateDirectory(absolute_remote_path);
                    }
                }
                //calling refresh
                刷新EToolStripMenuItem_Click(sender, e);
                ...
                foreach (var item in upload_list)
                {
                    if (!item.IsDir)
                    {
                        var relative_path = item.Path.Substring(local_path.Length + 1);
                        var absolute_remote_path = remote_path + relative_path;
                        var uploader = new Uploader(_pcsAPI, absolute_remote_path, item.Path);
        ...
        private void 分享ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            _asyncCall(delegate
            {
                ...
                {
                    data.Add(_file_list.GetData(path + item).FS_ID);
                }
                Invoke(new NoArgSTA(delegate
                {
